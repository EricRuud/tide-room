Tide Room third-party notices

These notices accompany components in Tide Room and its JUCE framework.
CHOW Tape magnetic-model adaptation: GPL-3.0-only; see CHOW-Tape-NOTICE.md.
Airwindows kWoodRoom: MIT; see Airwindows-MIT-LICENSE.txt.
MIT KEMAR measurements: see MIT-KEMAR-NOTICE.txt.
JUCE: AGPLv3 or the JUCE commercial licence; see JUCE-LICENSE.md.
Other files reproduce bundled dependency notices.

Original Tide Room code is licensed under AGPL-3.0-only unless a file states
other terms. Full project terms are in ../LICENSE.txt; component licenses
above remain in effect. Corresponding source and build instructions:
https://github.com/EricRuud/tide-room

Redistributions must include applicable notices and corresponding source
as required by these licenses.
