...
namespace tide::room {
void Tape::prepare(double sr,int block,int order) {
...
void Tape::setSettings(TapeSettings s) {
...
void Tape::process(juce::AudioBuffer<float>& buffer) {
    ...
    for(int offset=0;offset<buffer.getNumSamples();offset+=maximum){const int n=std::min(maximum,buffer.getNumSamples()-offset);
        ...
        if(!processing){oversampler->reset();for(auto& magnet:magnets)magnet.reset(false);preLow.fill(0);postInput.fill(0);postOutput.fill(0);dc.fill(0);toneLow.fill(0);processing=true;}
        ...
        for(size_t i=0;i<high.getNumSamples();++i){if(changing)cook();
            for(size_t c=0;c<2;++c){double x=high.getSample((int)c,(int)i);auto& magnet=magnets[c];
                preLow[c]=(1-prePole)*x+prePole*preLow[c];x=shelf*x+(1-shelf)*preLow[c];
                const double recorded=magnet.process(x*drive)/normalization;
