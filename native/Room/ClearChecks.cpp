...
void core(const juce::File& out){
    for(double rate:{44100.,48000.,96000.})for(float decay:{.25f,1.2f,4.f}){
        tide::room::ClearRoom room;room.prepare(rate,decay,.35f);
        juce::AudioBuffer<float> lead(2,4096);lead.clear();room.process(lead,decay,.35f,0);
        check(lead.getMagnitude(0,4096)==0,"Clear room creates noise on silence");
        juce::AudioBuffer<float> audio(2,(int)(rate*12));audio.clear();audio.setSample(0,0,.25f);audio.setSample(1,0,.25f);
        for(int offset=0;offset<audio.getNumSamples();offset+=127){juce::AudioBuffer<float> b(audio.getArrayOfWritePointers(),2,offset,std::min(127,audio.getNumSamples()-offset));room.process(b,decay,.35f,0);}
        double energy=0;float peak=0;
        for(int c=0;c<2;++c)for(int n=0;n<audio.getNumSamples();++n){const float v=audio.getSample(c,n);check(std::isfinite(v),"Core impulse nonfinite");energy+=v*v;peak=std::max(peak,std::abs(v));}
        check(energy>1.e-8&&peak<1&&room.guardCount()==0,"Core silent, unstable or guarded");
        check(audio.getMagnitude(0,audio.getNumSamples()-(int)rate,(int)rate)<1.e-6,"Core tail does not settle");
        std::cout<<"CLEAR CORE rate="<<rate<<" decay="<<decay<<" peak="<<peak<<" energy="<<energy<<"\n";
        if(rate==48000)save(out.getChildFile("impulse-decay-"+juce::String(decay,2)+".wav"),audio,rate);
    }
    for(float damping:{0.f,.35f,1.f}){
        tide::room::ClearRoom room;room.prepare(48000,1.2f,damping);juce::AudioBuffer<float> lead(2,4096);lead.clear();room.process(lead,1.2f,damping,0);
        juce::AudioBuffer<float> impulse(2,48000*6);impulse.clear();impulse.setSample(0,0,.25f);impulse.setSample(1,0,.25f);room.process(impulse,1.2f,damping,0);
        check(room.guardCount()==0,"Damping instability");save(out.getChildFile("impulse-damping-"+juce::String(damping,2)+".wav"),impulse,48000);
    }
    // Static processing must be linear and independent of callback partitioning.
    juce::AudioBuffer<float> reference(2,48000*3),partitioned(2,48000*3),quiet(2,48000*3);
    for(int c=0;c<2;++c)for(int n=0;n<reference.getNumSamples();++n){const float v=.03f*(float)(std::sin(n*(c?.313:.137))+.3*std::sin(n*.011));reference.setSample(c,n,v);partitioned.setSample(c,n,v);quiet.setSample(c,n,v*.0625f);}
    for(int variant=0;variant<3;++variant){tide::room::ClearRoom room;room.prepare(48000,1.2f,.35f);auto& audio=variant==0?reference:variant==1?partitioned:quiet;const int block=variant==1?127:1024;
        for(int offset=0;offset<audio.getNumSamples();offset+=block){juce::AudioBuffer<float> b(audio.getArrayOfWritePointers(),2,offset,std::min(block,audio.getNumSamples()-offset));room.process(b,1.2f,.35f,-3);}
    }
    float delta=0,linear=0;for(int c=0;c<2;++c)for(int n=0;n<reference.getNumSamples();++n){delta=std::max(delta,std::abs(reference.getSample(c,n)-partitioned.getSample(c,n)));linear=std::max(linear,std::abs(reference.getSample(c,n)-quiet.getSample(c,n)*16));}
    std::cout<<"CLEAR partition error="<<delta<<" scaling error="<<linear<<"\n";check(delta==0&&linear<1.e-6,"Clear room partitioning or linearity failed");
    tide::room::ClearRoom moving;moving.prepare(48000,1.2f,.35f);juce::AudioBuffer<float> tone(2,48000*6);double phase=0;
    for(int offset=0;offset<tone.getNumSamples();offset+=127){const int count=std::min(127,tone.getNumSamples()-offset);juce::AudioBuffer<float> b(tone.getArrayOfWritePointers(),2,offset,count);
        ...
        const float t=(float)offset/tone.getNumSamples();moving.process(b,2.125f+1.875f*(float)std::sin(t*30),.5f+.5f*(float)std::cos(t*24),-3);
    }
    check(moving.guardCount()==0&&tone.getMagnitude(0,tone.getNumSamples())<1,"Moving decay/damping instability");save(out.getChildFile("decay-damping-motion.wav"),tone,48000);
    // Same test process, same host rate: keep an unmodified wooden-core comparison.
    for(int kind=0;kind<2;++kind){tide::room::ClearRoom clear;tide::room::WoodRoom wood;clear.prepare(48000,1.2f,.35f);wood.prepare(48000,.88f,.36f);
        juce::AudioBuffer<float> impulse(2,48000*6),lead(2,4096);lead.clear();if(kind)wood.process(lead,.88f,.36f,0);else clear.process(lead,1.2f,.35f,0);
        impulse.clear();impulse.setSample(0,0,.25f);impulse.setSample(1,0,.25f);
        const auto begin=std::chrono::steady_clock::now();for(int offset=0;offset<impulse.getNumSamples();offset+=512){juce::AudioBuffer<float> b(impulse.getArrayOfWritePointers(),2,offset,std::min(512,impulse.getNumSamples()-offset));if(kind)wood.process(b,.88f,.36f,0);else clear.process(b,1.2f,.35f,0);}
        const double load=std::chrono::duration<double>(std::chrono::steady_clock::now()-begin).count()/6;
        std::cout<<(kind?"WOOD CORE":"CLEAR CORE")<<" callback average="<<load*100<<"%\n";save(out.getChildFile(kind?"compare-wood-impulse.wav":"compare-clear-impulse.wav"),impulse,48000);
    }
    std::cout<<"PASS clear core: rate/decay/damping, silence, linearity, partitioning and moving controls\n";
}
void lifecycle(RoomProcessor& p){
    p.set("tapeOn",0);p.set("clearDecay",4);p.set("clearTail",0);p.updateRoom();p.prepareToPlay(48000,512);p.play();
    ...
    p.selectWornPreset(0);p.set("wornNoise",1);p.set("clearDecay",1.2f);p.play();render(p,music,48000,512);p.stop();render(p,tail,48000,511);check(p.sleepingForTest()&&tail.getMagnitude(0,tail.getNumSamples()-48000,48000)==0,"Worn hiss prevented sleep");
...
int main(int argc,char* argv[]){juce::ScopedJuceInitialiser_GUI gui;std::cout<<std::unitbuf;
    try{check(argc>=2,"Usage: TideClearCheck NEW_OUTPUT [SAVED_SETTINGS]");juce::File out(argv[1]);check(!out.exists(),"Use a new output directory");out.createDirectory();
        if(argc>3&&juce::String(argv[3])=="--placement"){
            RoomProcessor p(false);p.set("tapeOn",0);p.set("output",-3);p.set("reflections",-6);p.set("clearTail",-6);p.set("p1_mute",1);p.set("p2_mute",1);p.selectVoice(0,6);p.set("p0_length",.16f);p.set("p0_brightness",.8f);
        ...
        {RoomEditor editor(p);png(out.getChildFile("clear-room.png"),editor.createComponentSnapshot(editor.getLocalBounds()));png(out.getChildFile("motion.png"),editor.motionPanelSnapshot());}
        for(int tapeMode=0;tapeMode<2;++tapeMode){p.setStateInformation(original.getData(),(int)original.getSize());if(tapeMode==0)p.set("tapeOn",0);p.updateRoom();p.prepareToPlay(48000,512);p.play();
            ...
            p.stop();juce::AudioBuffer<float> tail(audio.getArrayOfWritePointers(),2,48000*24,48000*6);render(p,tail,48000,1024);save(out.getChildFile(tapeMode?"clear-room-saved-tape.wav":"clear-room-tape-off.wav"),audio,48000);
        }
        ...
        p.set("clearDecay",2.82f);p.set("clearDamping",.61f);juce::MemoryBlock state;p.getStateInformation(state);RoomProcessor restored(false);restored.setStateInformation(state.getData(),(int)state.getSize());check(std::abs(restored.get("clearDecay")-2.82f)<.001&&std::abs(restored.get("clearDamping")-.61f)<.001&&!restored.isPlaying(),"Clear settings recall");
        for(double rate:{44100.,96000.}){p.stop();p.set("clearDecay",4);p.set("clearTail",0);p.set("output",0);p.set("evolution",1);p.set("tempo",160);for(int i=0;i<3;++i){p.set(RoomProcessor::partId(i,"level"),0);p.set(RoomProcessor::partId(i,"density"),1);p.set(RoomProcessor::partId(i,"length"),2.5f);}for(int i=0;i<6;++i){p.set(RoomProcessor::lfoId(i,"on"),1);p.set(RoomProcessor::lfoId(i,"rate"),i%2?4.f:.01f);p.set(RoomProcessor::lfoId(i,"depth"),1);}p.updateRoom();p.prepareToPlay(rate,512);p.play();juce::AudioBuffer<float> audio(2,(int)rate*6);const auto st=render(p,audio,rate,1024);check(st.peak>.001&&st.peak<1,"Rate/motion/sustain stress clipping");report(rate==44100?"STRESS 44.1k":"STRESS 96k",st);}
        p.setStateInformation(original.getData(),(int)original.getSize());lifecycle(p);p.releaseResources();
        std::cout<<"PASS eight patterns, saved scene, clear-state recall, 44.1/48/96 kHz and six-LFO extremes\n";return 0;
