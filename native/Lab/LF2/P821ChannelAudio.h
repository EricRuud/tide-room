...
public:
    struct Parameters {
        ...
        double lowFrequencyHz=50.,lowFrequencyQ=2.,lowFrequencyScale=2.;
    };
    ...
    void processParts(const float* input,double* pre,double* cap,double* width,
        ...
        if (!p_.stateVariable) for (int c=0;c<2;++c) for (int j=0;j<4;++j) {
            left[c][j]=coefficients(j,previous.values[c][j],previous.bandwidthOffset[c][j],previous.lowFrequencyMorph[c]);
            right[c][j]=coefficients(j,current.values[c][j],current.bandwidthOffset[c][j],current.lowFrequencyMorph[c]);
        }
        for (int n=0;n<256;++n) {
...
private:
    ...
    std::array<double,5> coefficients(int j,double value,double bandwidthOffset,double morph) const noexcept {
        constexpr double pi=3.1415926535897932384626433832795;
        ...
        if (j==0) {
            ...
            std::array<double,5> result {A*((A+1)-(A-1)*c+beta)/a0,2*A*((A-1)-(A+1)*c)/a0,
                    A*((A+1)-(A-1)*c-beta)/a0,-2*((A-1)+(A+1)*c)/a0,
                    ((A+1)+(A-1)*c-beta)/a0};
            if (p_.lowFrequencyCorrection) {
                const double amp=std::pow(10.,value*p_.lowFrequencyScale/40.);
                const double angle=2*pi*p_.lowFrequencyHz/48000.;
                const double qlf=std::clamp(p_.lowFrequencyQ*std::exp(bandwidthOffset),.2,8.);
                const double al=std::sin(angle)/(2*qlf),a=1+al/amp,b=-2*std::cos(angle)/a;
                const std::array<double,5> bell {(1+al*amp)/a,b,(1-al*amp)/a,b,(1-al/amp)/a};
                for (int k=0;k<5;++k) result[k]=result[k]*(1-morph)+bell[k]*morph;
            }
            return result;
        }
