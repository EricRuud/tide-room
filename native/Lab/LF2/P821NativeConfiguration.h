...
struct NativeConfiguration {
    ...
    bool load(const std::string& folder) {
        // JUCE supplies UTF-8. Native filesystem paths preserve Windows names
        // outside the current ANSI code page (including the user's home folder).
        const auto path=std::filesystem::u8path(folder);
        std::ifstream weights(path/"control.bin",std::ios::binary);
        ...
        std::ifstream featureFile(path/"features.bin",std::ios::binary);
        if (!read(featureFile,features)) return false;
        std::ifstream audioFile(path/"audio.bin",std::ios::binary);
        ...
        std::ifstream slowFile(path/"slow.bin",std::ios::binary);
        if (!read(slowFile,gentle)||std::abs(gentle[0]-47.)>1e-6) return false;
        slow={gentle[0],gentle[1],gentle[2],gentle[3],gentle[4]};
        std::ifstream kernelFile(path/"base.bin",std::ios::binary|std::ios::ate);
        ...
        std::ifstream acFile(path/"ac.bin",std::ios::binary);acEnabled=static_cast<bool>(acFile);
        ac={1.,0.,0.,0.,0.};if (acEnabled&&!read(acFile,ac)) return false;
        std::ifstream dynamicQFile(path/"dynamic-q.bin",std::ios::binary);controls.dynamicBandwidth=static_cast<bool>(dynamicQFile);
        if (controls.dynamicBandwidth&&!(dense(dynamicQFile,controls.bandwidth)&&read(dynamicQFile,controls.bandwidthTau))) return false;
        std::ifstream gainQFile(path/"gain-q.bin",std::ios::binary);audio.gainDependentBandwidth=static_cast<bool>(gainQFile);
        if (audio.gainDependentBandwidth&&!read(gainQFile,audio.bandwidthShape)) return false;
        std::ifstream svfFile(path/"svf.bin",std::ios::binary);audio.stateVariable=static_cast<bool>(svfFile);
        if (audio.stateVariable) {unsigned char flag=0;if (!svfFile.read(reinterpret_cast<char*>(&flag),1)||flag!=1) return false;}
        std::ifstream quietFastFile(path/"quiet-fast-state.bin",std::ios::binary);controls.settleFastAfterSilence=static_cast<bool>(quietFastFile);
        if (controls.settleFastAfterSilence&&!read(quietFastFile,controls.quietFastState)) return false;
        std::ifstream quietWidthFile(path/"quiet-width-state.bin",std::ios::binary);controls.settleWidthAfterSilence=static_cast<bool>(quietWidthFile);
        if (controls.settleWidthAfterSilence&&!read(quietWidthFile,controls.quietWidthState)) return false;
        std::ifstream restFile(path/"rest.bin",std::ios::binary);controls.resetDerivedAfterSilence=static_cast<bool>(restFile);
        ...
        std::ifstream lfFile(path/"lf-correction.bin",std::ios::binary);
        controls.lowFrequencyCorrection=audio.lowFrequencyCorrection=static_cast<bool>(lfFile);
        if (controls.lowFrequencyCorrection) {
            if (!dense(lfFile,controls.lowFrequencyHead)||audio.stateVariable||!controls.dynamicBandwidth) return false;
            std::array<double,5> lf;
            if (!read(lfFile,lf)) return false;
            for (double value:lf) if (!std::isfinite(value)) return false;
            if (lf[0]<0||lf[0]>2||lf[1]<0||lf[1]>1||lf[2]<25||lf[2]>110||lf[3]<.3||lf[3]>8||lf[4]<.2||lf[4]>4) return false;
            controls.lowFrequencyThreshold=lf[0];controls.lowFrequencyAmount=lf[1];
            audio.lowFrequencyHz=lf[2];audio.lowFrequencyQ=lf[3];audio.lowFrequencyScale=lf[4];
        }
        return true;
    }
};
