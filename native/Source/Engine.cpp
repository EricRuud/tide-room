...
float Engine::modulated(float phase,float modPhase,float depth,float folding,int mode,float index) noexcept {
    const float modulator=std::sin(modPhase);
    // Convex blends stay inside the original folder/saturator input bound.
    // Pure multiplication gives genuine bipolar ring modulation. AM retains
    // the carrier. Complex mode combines phase modulation and ring modulation.
    const float carrier=.80f*.35f*std::sin(phase)+.20f*fold(phase,folding);
    if(mode==4)return carrier*((1-depth)+depth*modulator);
    if(mode==5)return carrier*(1-.5f*depth+.5f*depth*modulator);
    return (.8f*.35f*std::sin(phase+index*modulator)+.2f*fold(phase,folding))
        *(1-.65f*depth+.65f*depth*modulator);
}
void Engine::prepare(double sr,int block,bool internalSpace,bool separateVoices) {
    stems=separateVoices?std::make_unique<std::array<Stem,voicesCount>>():nullptr;
#if TIDE_NO_CONVOLUTION
    juce::ignoreUnused(internalSpace);internalSpaceEnabled=false;
#else
    internalSpaceEnabled=internalSpace;
#endif
    ...
    wet.init(internalSpaceEnabled?settings.space:0,rate,.04); mode.init((float)settings.spaceMode,rate,.08);
    output.init(juce::Decibels::decibelsToGain(settings.output),rate,.025);
    mod.init(0,innerRate,.04);
    ratio.init(settings.modRatio,innerRate,.04);depth.init(settings.modDepth,innerRate,.025);
    for(auto& b:bend) b.init(0,innerRate,.012);
    reset();
}
void Engine::reset() {
    for(auto& v:voices) v=Voice{};
    if(stems)for(auto& stem:*stems){stem.ring.fill(0);stem.dc=0;stem.drain=0;}
    ...
    ratio.value=ratio.target;depth.value=depth.target;
#if !TIDE_NO_CONVOLUTION
    if(internalSpaceEnabled){close.reset(); bloom.reset();}
#endif
    panicRemaining=0;
}
...
void Engine::setSettings(Settings s) {
    ...
    release.target=juce::jlimit(.04f,4.f,s.release); wet.target=internalSpaceEnabled?juce::jlimit(0.f,1.f,s.space):0;
    mode.target=s.spaceMode==0?0.f:1.f;
    ratio.target=juce::jlimit(.125f,6.f,s.modRatio);depth.target=juce::jlimit(0.f,1.f,s.modDepth);
...
void Engine::midi(const juce::MidiMessage& m) {
    ...
    if(m.isNoteOn()) {
        ...
        *selected=Voice{}; selected->active=selected->held=true;
        selected->articulation=juce::jlimit(0,6,settings.articulation);
...
float Engine::nextInner(float* voiceSamples) {
    if(voiceSamples)std::fill_n(voiceSamples,voicesCount,0.f);
    ...
    const float modulationRatio=ratio.next(),modulationDepth=depth.next();
    ...
    for(auto& v:voices) {
        ...
        if(controlPosition==0) {
            ...
            if(v.articulation!=0) {
                ...
                const double cutoff=v.articulation>=4?std::min(rate*.42,300+frequency*(1.8+(1+12*t)*opening)+10000*opening)
                    :std::min(rate*.42,110+frequency*(1+(.8+9*t)*opening)+5000*opening);
                const float g=(float)(-std::expm1(-2*juce::MathConstants<double>::pi*cutoff/innerRate));
                v.filterStep=(g-v.filterG)/interval;
                const float index=v.articulation>=6?(.2f+2.2f*t)*modulationDepth*(.12f+.88f*end):(.25f+2.2f*t)*end;
        ...
        const float source=v.articulation>=4?modulated((float)v.phase,(float)v.modPhase,modulationDepth,folding,v.articulation,v.metalIndex)
            :v.articulation==3?metal((float)v.phase,(float)v.modPhase,v.metalIndex,folding):fold((float)v.phase,folding);
        ...
        if(v.articulation!=0) {
            ...
            v.modPhase+=v.phaseIncrement*(v.articulation>=4?modulationRatio*(1+.003f*mv*v.lifeSine):1.4);
        ...
        if(voiceSamples)voiceSamples[&v-voices.data()]=sample;
    ...
    const float headroom=std::sqrt(1+energy*energy);
    if(voiceSamples)for(int i=0;i<voicesCount;++i)voiceSamples[i]/=headroom;
    return sum/headroom;
}
void Engine::render(float* l,float* r,int count) {
    ...
    while(offset<count) {
...
#if !TIDE_NO_CONVOLUTION
            if(internalSpaceEnabled){workClose.setSample(0,i,mono); workClose.setSample(1,i,mono);
            workBloom.setSample(0,i,mono); workBloom.setSample(1,i,mono);}
#endif
        }
#if !TIDE_NO_CONVOLUTION
        auto c=juce::dsp::AudioBlock<float>(workClose);if(internalSpaceEnabled)c=c.getSubBlock(0,(size_t)n);
        auto b=juce::dsp::AudioBlock<float>(workBloom);if(internalSpaceEnabled)b=b.getSubBlock(0,(size_t)n);
        if(internalSpaceEnabled){close.process(juce::dsp::ProcessContextReplacing<float>(c));
        ...
        for(int i=0;i<n;++i) {
            ...
            if(internalSpaceEnabled){l[offset+i]=(l[offset+i]+amount*(nearGain*c.getSample(0,i)+bloomGain*b.getSample(0,i)))*gain;
...
void Engine::renderVoices(const std::array<float*,voicesCount>& outputs,int count) {
    juce::ScopedNoDenormals noDenormals;
    jassert(stems&&!internalSpaceEnabled);
    if(!stems)return;
    constexpr int taps=(int)constants::decimator.size();
    for(int offset=0;offset<count;offset+=maximum){const int n=std::min(maximum,count-offset);
        for(int i=0;i<n;++i){std::array<float,voicesCount> samples{},downsampled{};
            for(int v=0;v<voicesCount;++v){auto& stem=(*stems)[(size_t)v];if(voices[(size_t)v].active)stem.drain=385;else stem.drain=std::max(0,stem.drain-1);}
            for(int j=0;j<oversampling;++j){nextInner(samples.data());
                for(int v=0;v<voicesCount;++v){auto& ring=(*stems)[(size_t)v].ring;
                    ring[(size_t)ringPosition]=ring[(size_t)(ringPosition+taps)]=samples[(size_t)v];}
                if(++ringPosition==taps)ringPosition=0;
                if(j==0)for(int v=0;v<voicesCount;++v){if((*stems)[(size_t)v].drain==0)continue;const auto& ring=(*stems)[(size_t)v].ring;
#if JUCE_MAC
                    vDSP_dotpr(ring.data()+ringPosition,1,constants::decimator.data(),1,&downsampled[(size_t)v],taps);
#else
                    for(int k=0;k<taps;++k)downsampled[(size_t)v]+=ring[(size_t)(ringPosition+k)]*constants::decimator[(size_t)k];
#endif
                }
            }
            for(int v=0;v<voicesCount;++v){auto& stem=(*stems)[(size_t)v];const float value=downsampled[(size_t)v];
                stem.dc+=dcAlpha*(value-stem.dc);outputs[(size_t)v][offset+i]=value-stem.dc;}
        }
        bool clearAfterBlock=false;
        for(int i=0;i<n;++i){
#if !TIDE_NO_CONVOLUTION
            mode.next();wet.next();
#endif
            float gain=output.next();
            if(panicRemaining>0){gain*=(float)panicRemaining/(float)panicLength;if(--panicRemaining==0)clearAfterBlock=true;}
            else if(clearAfterBlock)gain=0;
            for(auto* out:outputs)out[offset+i]*=gain;
        }
        if(clearAfterBlock)reset();
    }
}
...
float Engine::ringProbe(double f,double r,float dep,float folding,float saturation,int mode,float index) {
    float result=0;
    for(int j=0;j<oversampling;++j){push(saturate(modulated((float)probePhase,(float)probeModPhase,dep,folding,mode,index),saturation));if(j==0)result=down();
        probePhase+=2*juce::MathConstants<double>::pi*f/innerRate;probeModPhase+=2*juce::MathConstants<double>::pi*f*r/innerRate;
        if(probePhase>=2*juce::MathConstants<double>::pi)probePhase-=2*juce::MathConstants<double>::pi;
        if(probeModPhase>=2*juce::MathConstants<double>::pi)probeModPhase-=2*juce::MathConstants<double>::pi;
    }return result;
}
